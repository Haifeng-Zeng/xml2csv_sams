...
def read_command_line_para() :
    ...
    parser.add_argument("--skip", "-s", action="store_true" , help="skip checking command file")
    parser.add_argument("first_file", help="Read command file")
    parser.add_argument("second_file", help="Read configuration file")
    if platform.system() == "Windows" :
        ...
        sys.exit(1)
    # for windows
    #parser.add_argument("third_files", help="Read response file")
    #for linux
    #parser.add_argument("response_files", nargs='+', help="Read response file")
    
    args = parser.parse_args()

    if args.skip :
        conf_file, res_files = args.first_file, args.second_file
    else :
        req_file, conf_file, res_files = args.first_file, args.second_file, args.third_files
    if platform.system() == "Windows" :
        res_files = glob.glob(args.third_files)
#    print("RES: ", res_files)
    print("",conf_file, res_files)
    if args.skip :
        print("",conf_file, res_files)
        return(conf_file, res_files)
    else :
        print(req_file, conf_file, res_files)
        return(req_file, conf_file, res_files)
...
def read_command_line_args() :
    ...
    args = parser.parse_args()
#    print(args)
    cmd_file, conf_file, output_file, res_files = args.cmd_file, args.conf_file, args.output_file, args.res_files

    return(cmd_file, conf_file, output_file, res_files)
...
def write_file_266(write_list, file_name, conf_file) :
    ...
    header.append("NE_ID")
    for i in range(len(lines)) :
        header.append(para_dict[lines[i]][1])
