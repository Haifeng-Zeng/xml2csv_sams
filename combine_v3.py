...
def combine_dict(join_dict, output_dict, file1_list, file2_list) :
    ...
    key_join_length = min(key_join_length, len(join_list_f1))
    f2_dict = dict()
    ...
    print("index created",  time.time() - T1)
    ...
    for item1 in file1_list :
        ...
        tmp_item1 = dict()
        for k in join_list_f1 :
            tmp_k = k.replace("f1.","f2.",1)
            tmp_item1[tmp_k] = item1[k]
        item2 = dict()
        if key in f2_dict.keys() :
            # loop again
            tmp_item2 = dict()
            for item in f2_dict[key] :
                for k in join_list_f2:
                    tmp_item2[k] = item[k]
    ...
    print("total combine_dict", time.time() - T1)
