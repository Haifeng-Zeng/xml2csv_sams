...
class configFile :
    ...
    def __init__(self, file_name) :
        ...
        self.join_dict = dict()
        self.f1_join_fields = list()
        self.f2_join_fields = list()
        self.alias = dict()
        ...
        for line in lines :
            ...
            if arr[0].strip() == "fields" :
                ...
                for field in fields :
                    ...
                    if file == "f1" :
                        self.f1_join_fields.append(field)
                    elif file == "f2" :
                        self.f2_join_fields.append(field)
            elif arr[0] == "join" :
                #set join
                ...
                if file_no == "f1" :
                    key = f1field
                    if f1field not in self.f1_join_fields :
                        self.f1_join_fields.append(f1field)
                elif file_no == "f2" :
                    value = f1field
                    if f1field not in self.f2_fields :
                        self.f2_join_fields.append(f1field)
                else :
                ...
                if file_no == "f1" :
                    key = f2field
                    if f2field not in self.f1_join_fields :
                        self.f1_join_fields.append(f2field)
                elif file_no == "f2" :
                    value = f2field
                    if f2field not in self.f2_join_fields :
                        self.f2_join_fields.append(f2field)
                else :
                    print("Error in join part of conf file")
                    sys.exit(1)
        ...
        print("generate config: ", time.time() - T1)
...
def orgnize_list(list_of_dict, f2_join_fields) :
...
def combine_dict(join_dict, output_dict, file1_list, f2_dict) :
...
file1_list = read_csv(file1_name, configer.f1_delimiter, configer.f1_join_fields, 1)
file2_list = read_csv(file2_name, configer.f2_delimiter, configer.f2_join_fields, 2)
#print(file2_dict)

#write_list = combine_dict(configer.join_dict, configer.output_dict, file1_list, file2_dict)

file2_dict = orgnize_list(file2_list, configer.f2_join_fields)
#write_list = combine_dict(configer.join_dict, configer.output_dict, file1_list, file2_dict)
